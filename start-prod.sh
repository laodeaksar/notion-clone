...
# Build allowed origins from REPLIT_DOMAINS (production domain)
REPLIT_ORIGIN=""
if [ -n "$REPLIT_DOMAINS" ]; then
  REPLIT_ORIGIN="https://$(echo "$REPLIT_DOMAINS" | cut -d',' -f1)"
fi

if [ -n "$ALLOWED_ORIGINS" ]; then
  COMPUTED_ORIGINS="$ALLOWED_ORIGINS"
elif [ -n "$REPLIT_ORIGIN" ]; then
  COMPUTED_ORIGINS="${REPLIT_ORIGIN}"
else
  COMPUTED_ORIGINS="http://localhost:5000"
fi

# Write .dev.vars for each wrangler service
write_dev_vars() {
  local dir=$1
...
# Start CF Worker services via wrangler dev (local mode)
...
# Start SvelteKit production server (built output)
echo "Starting SvelteKit production server..."
cd apps/web
ORIGIN="${REPLIT_ORIGIN:-http://localhost:5000}" PORT=5000 node build/index.js
