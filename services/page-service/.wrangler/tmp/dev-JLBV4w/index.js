...
// src/repository/page.repo.ts
function createPageRepo(db) {
  return {
    /**
    ...
    async findAll(userId, parentId) {
      const condition = parentId ? and(eq(pages.userId, userId), eq(pages.parentId, parentId)) : eq(pages.userId, userId);
      return db.select().from(pages).where(condition);
    },
    /**
    ...
    async findById(id, userId) {
      const rows = await db.select().from(pages).where(and(eq(pages.id, id), eq(pages.userId, userId)));
      return rows[0] ?? null;
    },
    /**
    ...
    async update(id, input, userId) {
      ...
      const [page] = await db.update(pages).set({
        ...
        updatedAt: now
      }).where(and(eq(pages.id, id), eq(pages.userId, userId))).returning();
      return page ?? null;
    },
    async delete(id, userId) {
      const rows = await db.delete(pages).where(and(eq(pages.id, id), eq(pages.userId, userId))).returning();
      return rows.length > 0;
    }
  };
}
__name(createPageRepo, "createPageRepo");
...
var PageInputSchema2 = object({
  title: pipe(string(), minLength(1)),
  parentId: optional(nullable(string()))
});
var PageUpdateSchema2 = object({
  title: optional(pipe(string(), minLength(1))),
  parentId: optional(nullable(string()))
});
...
function createPagePublisher(queue) {
...
function createPageService(db, eventsQueue) {
  ...
  return {
    async getPages(userId, parentId) {
...
__name(createPageService, "createPageService");
...
// src/routes/pages.ts
var pageRoutes = new Hono2().use("*", authMiddleware).get("/", async (c) => {
  const userId = c.var.userId;
  const db = createDb(c.env.DATABASE_URL);
  const svc = createPageService(db, c.env.EVENTS_QUEUE);
  const parentId = c.req.query("parentId");
  const pages2 = await svc.getPages(userId, parentId);
  return c.json({ pages: pages2 });
...
var searchRoutes = new Hono2().use("*", authMiddleware).get("/", async (c) => {
  const q = c.req.query("q")?.trim() ?? "";
  if (q.length < 2) {
    return c.json({ query: q, results: [] });
  }
  const db = createDb(c.env.DATABASE_URL);
  const results = await db.select().from(searchIndex).where(ilike(searchIndex.body, `%${q}%`)).orderBy(desc(searchIndex.updatedAt)).limit(50);
  return c.json({ query: q, results });
});

// src/routes/index.ts
var routes = new Hono2().get("/", (c) => c.json({ status: "ok", service: "page-service" })).route("/pages", pageRoutes).route("/search", searchRoutes);

// src/queue/handlers.ts
async function handlePageEvent(event, env2) {
...
__name(handlePageEvent, "handlePageEvent");
...
var src_default = {
  ...
  async queue(batch, env2) {
    for (const msg of batch.messages) {
      try {
        await handlePageEvent(msg.body, env2);
        msg.ack();
      } catch (err) {
...
// .wrangler/tmp/bundle-DFByHz/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-DFByHz/middleware-loader.entry.ts
