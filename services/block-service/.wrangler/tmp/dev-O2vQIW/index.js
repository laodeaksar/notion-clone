...
// src/repository/block.repo.ts
function createBlockRepo(db) {
  return {
    ...
    async update(id, input) {
      const now = /* @__PURE__ */ new Date();
      const [block] = await db.update(blocks).set({ ...input, updatedAt: now }).where(eq(blocks.id, id)).returning();
      return block ?? null;
    },
    async delete(id) {
      const rows = await db.delete(blocks).where(eq(blocks.id, id)).returning();
      return rows.length > 0;
    }
  };
}
__name(createBlockRepo, "createBlockRepo");

// ../../packages/shared/src/types/block.types.ts
var BlockInputSchema2 = object({
  id: optional(string()),
  pageId: string(),
  type: string(),
  content: unknown(),
...
function createBlockPublisher(queue) {
...
__name(createBlockService, "createBlockService");
...
var blockRoutes = new Hono2().use("*", authMiddleware).get("/", async (c) => {
  const pageId = c.req.query("pageId");
  if (!pageId) return c.json({ error: "pageId is required" }, 400);
  const db = createDb(c.env.DATABASE_URL);
  const svc = createBlockService(db, c.env.EVENTS_QUEUE);
  const blocks2 = await svc.getBlocksByPage(pageId);
  return c.json({ blocks: blocks2 });
}).get("/:id", async (c) => {
  const db = createDb(c.env.DATABASE_URL);
  const svc = createBlockService(db, c.env.EVENTS_QUEUE);
...
async function handleBlockEvent(event, env2) {
  ...
  switch (event.type) {
    case "block.created":
    case "block.updated": {
      const updated = await db.update(pages).set({ updatedAt: /* @__PURE__ */ new Date() }).where(eq(pages.id, pageId)).returning();
      if (!updated.length) {
...
__name(handleBlockEvent, "handleBlockEvent");
...
var src_default = {
  ...
  async queue(batch, env2) {
    for (const msg of batch.messages) {
      try {
        await handleBlockEvent(msg.body, env2);
        msg.ack();
      } catch (err) {
...
// .wrangler/tmp/bundle-s1NuY5/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-s1NuY5/middleware-loader.entry.ts
